...
class FermionicDMSource(fd_nest.nestNRSource):
    def __init__(self, *args, mass_keV=57966, **kwargs):
        if ('detector' not in kwargs):
            kwargs['detector'] = 'default'

        df_fermionic_DM = pd.read_pickle(os.path.join(os.path.dirname(__file__), f'fermionic_DM_spectra/{mass_keV}_keV.pkl'))
